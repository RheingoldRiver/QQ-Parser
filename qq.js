$(function() {
	...
	window.fetchAsync = async (url) => {
		let response = await fetch(url);
		let data = await response.json();
		return data;
	}
	
	window.getMatchIds = async (input) => {
	...
	window.getWikiDataTable = async page => {
	...
	window.queryDdragon = async patch => {
		let urls = [
	...
	window.getGameData = async (games, keyToNameLookups) => {
		let ids = [];
	...
	window.processGame = async (gameId) => {
	...
	window.dataToString = (gameFull, runeData, clans, keyToNameLookups) => {
		let players = [
	...
	window.getName = (name, team) => {
		let replace = '^' + team.replace('.','');
		let re = new RegExp(replace);
		return name.replace(re, '');
	}
	
	window.getLink = (name, disambigs) => {
		if (disambigs[name]) return disambigs[name];
		return;
	}
	
	window.generatePlayerOutput = (players, data, order) => {
		let output = []
		for (let i in players) {
	...
	window.getTeamnames = (gameData, clanData, lookup) => {
	...
	window.getTeamnamesFromClans = (clanData, lookup) => {
	...
	window.generateTeamOutput = (teams, data, order, teamnames) => {
		let output = []
		for (let i in teams) {
			let outputThis = [];
			let arg = teams[i].arg;
	...
	window.generateGameOutput = (data, order) => {
		let output = []
		for (let key of order) {
	...
	window.processSeriesTotals = (printedData, input, event) => {
		let output = [];
	...
	window.getPatch = async () => {
		window.patch = await getWikiData('Maintenance:LPL_Patch');
